...
public class AccountTransactionService {
        ...
        synchronized (AccountTransactionService.class) {
            ...
            if (accountInfo == null) {
                throw new IllegalArgumentException(
                        String.format("Account #%d don't exit", account));
            }
            ...
            try {
                final BankAccountTransactionInfo creditTxn = new BankAccountTransactionInfo.Builder()
                        .accountId(accountInfo.getAccountNumber()).amount(amount).time(txnTime)
                        .txnId(generateTxnId()).txnType(TransactionType.CREDIT).build();
        ...
        synchronized (AccountTransactionService.class) {
            ...
            try {
                final BankAccountTransactionInfo debitTxn = new BankAccountTransactionInfo.Builder()
                        .accountId(accountInfo.getAccountNumber()).amount(amount).time(txnTime)
                        .txnId(generateTxnId()).txnType(TransactionType.DEBIT).build();
    ...
    public long transfer(long sourceAcnt, long sinkAcnt, double amount) {
        synchronized (AccountTransactionService.class) {
            ...
            if (source == null) {
                throw new IllegalArgumentException(
                        String.format("Source Account #%d don't exit", sourceAcnt));
            }

            final BankAccountInfo sink = accountService.getAccountInfo(sinkAcnt);
            if (sink == null) {
                throw new IllegalArgumentException(
                        String.format("Destination Account #%d don't exit", sinkAcnt));
            }

            final double sourceBalance = source.getBalance();
            if (sourceBalance == 0 || sourceBalance < amount) {
                throw new InsufficientFundsException(
                        "Insufficient Funds in Account #" + sourceAcnt);
            }
            final double sinkBalance = sink.getBalance();

            final double sourceDebit = amount;
            final double sinkCredit = getSinkAmount(amount, source.getCurrency(),
                    sink.getCurrency());

            final Date txnTime = new Date();
            try {
                final BankAccountTransactionInfo sourceDebitTxn = new BankAccountTransactionInfo.Builder()
                        .txnId(generateTxnId()).accountId(sourceAcnt).amount(sourceDebit)
                        .time(txnTime).txnType(TransactionType.DEBIT).build();
                final BankAccountTransactionInfo sinkCreditTxn = new BankAccountTransactionInfo.Builder()
                        .txnId(generateTxnId()).accountId(sinkAcnt).amount(sinkCredit).time(txnTime)
    ...
    private Double getSinkAmount(Double amount, Currency fromCurrency, Currency toCurrency) {
        if (!fromCurrency.equals(toCurrency)) {
            ...
            logger.info("Conversion of {} {} to {} :: {} at exchange rate {}/{}", amount,
                    fromCurrency.name(), toCurrency.name(), roundedValue, sourceFxRate, sinkFxRate);
            return roundedValue;
        }
