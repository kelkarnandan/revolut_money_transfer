...
public class AccountInfoDataService {
    ...
    public void addAccount(BankAccountInfo accountInfo) {
        if (getAccountInfo(accountInfo.getAccountNumber()) != null) {
            throw new UnsupportedOperationException(
                    "Account already exist #" + accountInfo.getAccountNumber());
        }
        accounts.add(accountInfo);
    }

    public void deleteAccount(long id) {
        final Optional<BankAccountInfo> accnt = accounts.stream()
                .filter(a -> id == a.getAccountNumber()).findAny();
    ...
    public final BankAccountInfo getAccountInfo(long id) {
        final Optional<BankAccountInfo> bankAccountInfo = accounts.stream()
                .filter(a -> id == a.getAccountNumber()).findAny();
